...
def from_log_probs(
    ...
    discounts, rewards, values, bootstrap_value,
    clip_rho_threshold=1.0, clip_pg_rho_threshold=1.0,
    name='vtrace_from_log_probs'):
  behaviour_action_log_probs = tf.convert_to_tensor(behaviour_action_log_probs, dtype=tf.float32)
  target_action_log_probs = tf.convert_to_tensor(target_action_log_probs, dtype=tf.float32)

  # Make sure tensor ranks are as expected.
  # The rest will be checked by from_action_log_probs.
  behaviour_action_log_probs.shape.assert_has_rank(2)
  target_action_log_probs.shape.assert_has_rank(2)

  with tf.name_scope(name, values=[behaviour_action_log_probs, target_action_log_probs, discounts, rewards, values, bootstrap_value]):
    log_rhos = target_action_log_probs - behaviour_action_log_probs
    vtrace_returns = from_importance_weights(
        log_rhos=log_rhos,
        discounts=discounts,
